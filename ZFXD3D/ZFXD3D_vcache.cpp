...
HRESULT ZFXD3DVCManager::CreateStaticBuffer(ZFXVERTEXID VertexID,
	UINT nSkinID, UINT nVerts, UINT nIndis,
	const void *pVerts, const WORD *pIndis,
	UINT *pnID)
{
	HRESULT  hr;
	DWORD    dwActualFVF;
					...
					if (pSkin->nTexture[iT] != MAX_ID)
					{
						m_pDevice->SetTexture(iT, (LPDIRECT3DTEXTURE9)
							m_pSkinMan->m_pTextures[
								...
								// 对 color1 和 color2 执行的操作，作为本stage的输出
	...
	UINT  SkinID) 
{
	...
	// 寻找在该类型的缓冲器中，是否已经有某些顶点使用相同的Skin
	// 如果找到，那么只需要将目标顶点数据加入到对应缓冲区即可，否则，创建一个新的使用目标Skin的顶点缓冲
